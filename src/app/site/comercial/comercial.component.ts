...
export class ComercialComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.subscriptionResolver = this.route.data.subscribe(
      resolverRoute => {
        if (resolverRoute.response.status === 200) {
          this.collectionSizeImoveis =
            resolverRoute.response.body.collectionSize;
          this.imoveis = resolverRoute.response.body.content;
        }
      },
      err =>
        console.log(
          "Error at Comercial:component ngOnInit:method. ERROR: ",
          err
        )
    );
  }
  ...
  onPageChanges(event) {
    ...
    this.subscriptionPages = this.imovelService
      .getAllComercial(page)
      .subscribe(res => {
        if (res.status === 200) {
  ...
  onFilter() {
    this.subscriptionFilter = this.imovelService
      .filterComercial(this.filterComercial.value)
      .subscribe(
        resp => {
          if (resp.status === 200) {
            this.collectionSizeImoveis = resp.body.collectionSize;
